...
- `tools\publish.ps1` validates the versioned Release archive and checksum, bundled documentation, Nexus metadata, BBCode listing source, and tracked `release\nexus-publish.json` identifiers. Without `-Publish`, it is a local preflight that neither reads credentials nor makes network requests. With `-Publish`, it requires `NEXUS_API_KEY` in the current shell and PowerShell confirmation, validates the target file is active, uploads/finalizes the archive, waits for availability, creates the selected file/version, and appends the changelog. For an existing Nexus file, supply `-Changelog`; the tracked main-file ID is used by default and may be overridden with `-ModFileId`. For a new file, use `-PublishMode NewFile -ModId` instead. Use `-Publish -WhatIf` to test the confirmation path without publishing.
