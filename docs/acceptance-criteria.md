...
| Logging and diagnostics | Runtime diagnostics are routed through `FabricLog` and preserve useful error context in debug builds without adding a release dependency. | Pass | Debug defaults to TRACE through ERROR and uses the `FTLog` backend; Release uses a generated no-op backend with no native logging calls and compiles without `Logs.reds`. |
| Package verification | Debug and Release archives contain the required FABRIC service, build profile, and selected logging backend. | Pass | `tools\package.ps1` extracts each archive, verifies the generated files, and rejects a release archive containing native logging calls. |
